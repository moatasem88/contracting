...
form_grid_templates = {"items": "templates/form_grid/contract_document_item_grid.html"}
...
class ContractDocument(SellingController):
    ...
    def calculate_totals(self):
        # Items here are Tender Item rows (contracting_item/description, no
        # item_code) - SellingController's full tax/total engine assumes
        # real Item master rows and throws on this shape, so totals are a
        # plain qty*rate rollup instead of calculate_taxes_and_totals().
        total = 0.0
        ...
        for d in self.get("items"):
            ...
            d.base_amount = d.amount * conversion_rate
            total += d.amount

        self.total = total
        self.base_total = total * conversion_rate
        self.net_total = self.total
        self.base_net_total = self.base_total
        self.grand_total = self.net_total
        self.base_grand_total = self.base_net_total
...
def create_sales_invoice(source_name, target_doc=None):
    ...
    def update_item(source_doc, target_doc, source_parent):
        target_doc.item_name = frappe.get_value("Item", source_doc.contracting_item, "item_name")
        target_doc.uom = frappe.get_value("Item", source_doc.contracting_item, "stock_uom")
        target_doc.income_account = default_income_account
...
def create_purchase_invoice(source_name, target_doc=None):
    ...
    def update_tax(source_doc, target_doc, source_parent):
        target_doc.category = "Total"
        target_doc.add_deduct_tax = "Add"
